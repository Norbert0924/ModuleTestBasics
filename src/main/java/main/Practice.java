...
public class Practice {
    ...
    public static boolean isShipAbleToFight(int shieldCharge, int weaponCharge) {
        ...
        double meanAllCharge = (double) (shieldCharge + weaponCharge) / 2;

        boolean ableToFight=shieldCharge > 20 && weaponCharge > 20 && meanAllCharge > 50;
    ...
    public static char[] reverseMessage(char[] message) {
        ...
        for (char i = 0; i < copy.length / 2; i++) {
    ...
    public static int getWorstMonthIndex(int[][] lossesPerMonths) {
        ...
        int worstHypothetical = Integer.MIN_VALUE;
        ...
        for (int i = 0; i < lossesPerMonths.length; i++) {
            ...
            if (sum > worstHypothetical) {
                worstHypothetical = sum;
                index = i;
            }
        }
        return index;
        ...
        for (int i = 0; i < shipPowers.length; i++) {
            for (int j = 0; j < shipPowers.length - 1 - i; j++) {
                ...
                if (shipPowers[j] < shipPowers[j + 1]) {
                    ...
                    String exchange = shipNames[j];
                    shipNames[j] = shipNames[j + 1];
                    shipNames[j + 1] = exchange;
                }
            }
        }
        for (int i = 0; i < shipPowers.length; i++) {
            System.out.println(shipNames[i] + " ereje " + shipPowers[i]);
        }
    }
